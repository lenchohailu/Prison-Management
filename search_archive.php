...
// Start session if needed (uncomment if required)
// session_start();
    ...
    <div class="container-fluid" style="margin-top:80px;">
        ...
        if (isset($_POST['search'])) {
            ...
            if (empty($searchP)) {
                ...
                $stmt = $conn->prepare("SELECT * FROM archive WHERE prison_ID LIKE ? OR prison_fname LIKE ? OR prison_mname LIKE ? OR prison_lname LIKE ? OR prison_age LIKE ? OR prison_gen LIKE ? OR prison_recored LIKE ?");
                $likeSearch = "%" . $searchP . "%";
                ...
                if ($count == 0) {
                    echo "<div style='font-size:24px; color:red; margin-left:400px;'>There are no search results</div>";
                } else {
                    // Paging setup
                    if (isset($_GET["page"])) {
                    ...
                    // Re-execute query with LIMIT for paging
                    $stmt_paged = $conn->prepare("SELECT * FROM archive WHERE prison_ID LIKE ? OR prison_fname LIKE ? OR prison_mname LIKE ? OR prison_lname LIKE ? OR prison_age LIKE ? OR prison_gen LIKE ? OR prison_recored LIKE ? LIMIT ? OFFSET ?");
                    $stmt_paged->bind_param("sssssssi", $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $likeSearch, $endlimit, $start_from);
                    $stmt_paged->execute();
